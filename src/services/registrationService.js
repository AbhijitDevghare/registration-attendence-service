...
class RegistrationService {
 ...
 static async getUserRegistrations(userId) {
  ...
  if (eventIds.length > 0) {
    const response = await axios.get(`${EVENT_SERVICE_URL}/getEventsByIds`, {
      params: { ids: eventIds }
    });
  ...
  // Get all registrations for an event
   static async getEventRegistrations(eventId) {
    ...
    const response = await axios.post(`${USER_SERVICE_URL}/getUsersByUserIds`, {
      userIds,
    });
  ...
  static async checkIn(userId, checkinMethod, proofUrl = null) {
    ...
    return attendance;
          ...
          if (eventIds.length > 0) {
            const response = await axios.get(`${EVENT_SERVICE_URL}/events/getEventsByIds`, {
              params: { ids: eventIds }
            });
            events = response.data;
          }
